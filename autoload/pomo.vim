...
" ICON | TIME_REMAINING | DONE_TODAY
function! pomo#status_bar() abort
    ...
    let l:show_done_today = get(g:, 'pomodoro_show_done', 1)
    let l:pomos = pomo#get_num_pomos_today()
    if s:pomo_status == 0
        return (l:use_icons ? s:pomo_idle_icon : '')
    elseif s:pomo_status == 1
        return (l:use_icons ? s:pomo_ongoing_icon : '') .
                    \ (empty(s:pomo_name) ? '' : s:pomo_name) .
                    \ (l:use_icons ? '' : '') .
                    \ (l:show_time ? '|' . pomo#remaining_time() : '') .
                    \ (l:show_done_today ? '|' . l:pomos : '')
    elseif s:pomo_status > 1
        if s:pomo_status == 2
            return (l:use_icons ? s:pomo_shortpause_icon : 'short break') .
                        \ (l:show_time ? '|' . pomo#remaining_time() : '') .
                        \ (l:show_done_today ? '|' . l:pomos : '')
        else
            return (l:use_icons ? s:pomo_longpause_icon : 'long break') .
                        \ (l:show_time ? '|' . pomo#remaining_time() : '') .
                        \ (l:show_done_today ? '|' . l:pomos : '')
        endif
    endif
